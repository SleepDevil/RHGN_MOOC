...
class dataloader():
    def __init__(self,data_dir=None,):
        ...
        # df_video = pd.read_csv('{}/ad_feature.csv'.format(data_dir))
        # print(df_video.info(verbose=True, null_counts=True))
        # df_video.dropna(axis=0, subset=['course_name', 'video_id', 'chapter'], inplace=True)  # 删除必要属性为空的样本
        #
        # print(df_video.info(verbose=True, null_counts=True))
        ...
        # df_click.drop_duplicates(subset=['user', 'course_id'], inplace=True, ignore_index=True)    #删除重复点击样本可以减少图中重复的边
        #
        # #删除没交互的行为
        ...
        # items = set(df_click.course_id.tolist())
        # df_user= df_user[df_user['userid'].isin(users)]
        # df_video= df_video[df_video['course_id'].isin(items)]
        ...
        # df_video.to_csv('{}/item_info'.format(data_dir), index=False)
        # df_click.to_csv('{}/click'.format(data_dir), index=False)
        # exit()
        df_user=pd.read_csv('{}/user.csv'.format(data_dir))
        df_video=pd.read_csv('{}/course.csv'.format(data_dir))
        df_click=pd.read_csv('{}/user-video.csv'.format(data_dir))
        df_user = df_user.astype({'user_id': 'str'}, copy=False)
        df_video = df_video.astype({'course_id': 'str', 'course_name': 'str', 'video_id': 'str', 'chapter': 'str'},
                                 copy=False)
        df_click = df_click.astype({'user_id': 'str', 'video_id': 'str'}, copy=False)

        self.user_dic = {k: v for v, k in enumerate(df_user.user_id)}
        self.video_dic = {k: v for v, k in enumerate(df_video.video_id.drop_duplicates())}  # 构建字典，删除重复项
        self.course_dic = {k: v for v, k in enumerate(df_video.course_id.drop_duplicates())}
        self.chapter_dict = {k: v for v, k in enumerate(df_video.chapter.drop_duplicates())}
        self.item_dic = {}
        self.c1,self.c2,self.c3=[],[],[]
        for i in range(len(df_video)):
            try:
                k=df_video.at[i,'video_id']
                v=i
                self.item_dic[k]=v
                self.c1.append(self.video_dic[df_video.at[i,'video_id']])
                self.c2.append(self.course_dic[df_video.at[i,'course_id']])

                self.c3.append(self.chapter_dict[df_video.at[i,'chapter']])

            except:
                continue
        print(min(self.c1),min(self.c2),min(self.c3))
        print(len(self.video_dic),len(self.course_dic),len(self.chapter_dict))
        df_click=df_click[df_click['video_id'].isin(self.video_dic)]
    ...
    def generate_pure_graph(self,is_save=True):
        ...
        click_item = [self.item_dic[item] for item in self.df_click.video_id ]
        data_dict = {
        ...
        temp = {k: model.get_sentence_vector(v) for v, k in self.video_dic.items()}
        cid1_feature = torch.tensor([temp[k] for _, k in self.video_dic.items()])

        temp = {k: model.get_sentence_vector(v) for v, k in self.course_dic.items()}
        cid2_feature = torch.tensor([temp[k] for _, k in self.course_dic.items()])

        temp = {k: model.get_sentence_vector(v) for v, k in self.chapter_dict.items()}
        cid3_feature = torch.tensor([temp[k] for _, k in self.chapter_dict.items()])


        '''将标签传进label'''
        label_gender = self.df_user.gender
        label_age = self.df_user.year_of_birth
    ...
    def generate_homo_graph(self,is_save=True):
        ...
        items=self.df_video.course_id.tolist()
        ...
        user_item_dic = neighbormap(self.df_click,init_dic , self.user_dic,new_item_dic,col_item='course_id')
        ...
        co_click_user = combination(self.df_click,users,col_item='course_id')
        ...
        label_age =  new_df_user.year_of_birth.tolist()
        G.nodes['user'].data['gender'] = torch.tensor(label_gender[:G.number_of_nodes('user')])
        G.nodes['user'].data['age'] = torch.tensor(label_age[:G.number_of_nodes('user')])
